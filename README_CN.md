...
**关键代码段：**
- **参数解析** (第 22-64 行)：手动解析 `--model`、`--base-url`、`-k` 等参数
- **Agent 实例化** (第 108-114 行)：创建 Agent 并配置默认系统提示和工具
- **事件监听** (第 122-179 行)：订阅 agent 事件，实现：
  - `message_delta`：实时打印 AI 回复文本
  - `tool_start` / `tool_end`：显示工具执行状态
  - `message_end`：处理错误信息
- **REPL 交互** (第 182-229 行)：使用 Node.js `readline` 模块，处理 `/reset` 和 `/messages` 命令
